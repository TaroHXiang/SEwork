...
1. 准备或修改 JSONL
2. 先执行 `python scripts/build_knowledge_index.py --diagnose-only --log-dir logs` 确认环境与文档统计正常
3. 再执行 `python scripts/build_knowledge_index.py --batch-size 256 --log-dir logs --resume`
4. 确认生成了 `knowledge/index/knowledge.faiss` 和 `knowledge/index/metadata.json`
5. 如遇中断或后半程失败，可直接重复执行上一条带 `--resume` 的命令恢复
6. 重启应用，使新的知识索引生效

说明：

- 构建日志默认写入 `logs/knowledge-index-*.log`
- checkpoint 默认位于 `knowledge/index/.checkpoint/`
- 如果你希望显式指定恢复目录，可使用 `--checkpoint-dir knowledge/index/.checkpoint`
