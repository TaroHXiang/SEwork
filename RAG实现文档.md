...
- `knowledge/source/cell_marker_kb_filtered.jsonl`（CellMaker 3.0 数据集精简版，36,401 条，丢弃冗余 marker 条目）
...
```powershell
# 推荐正式构建命令（CPU 环境，batch_size=256）
# - 持久化日志输出到 logs/knowledge-index-*.log
# - 存在兼容 checkpoint 时自动恢复
...
# 显式指定 checkpoint 目录，适合反复恢复长任务
python scripts/build_knowledge_index.py --batch-size 256 --log-dir logs --checkpoint-dir knowledge/index/.checkpoint --resume

# 完整参数
python scripts/build_knowledge_index.py --batch-size 256 --output-dir knowledge/index --model BAAI/bge-small-zh-v1.5 --log-dir logs --checkpoint-dir knowledge/index/.checkpoint --resume
```

**日志与 checkpoint 说明：**

- 推荐总是显式带上 `--log-dir logs`，这样每次运行都会留下可回溯的构建日志
- 默认 checkpoint 位于 `knowledge/index/.checkpoint/`
- 当 embedding 已完成但写索引或写元数据失败时，可用 `--resume` 直接复用已有 checkpoint，避免从头重算
- 如果只是想先确认环境是否可用，优先运行 `--diagnose-only --log-dir logs`
...
3. 重新构建索引：`python scripts/build_knowledge_index.py --batch-size 256 --log-dir logs --resume`
